...
def _scrape_csrf(scraper, page_url):
    ...
    """
    try:
        r    = scraper.get(page_url, headers={"Accept": "text/html,*/*;q=0.9", "Referer": BASE_URL})
        ...
        logger.warning(f"[CSRF_SCRAPE] gagal dari {page_url}: {e}")
    return None
...
def do_request(account, method, url, data=None, headers=None):
    ...
    for attempt in range(3):
        ...
        if method.upper() != "GET" and csrf_source_page:
            fresh_csrf = _scrape_csrf(scraper, csrf_source_page)
            if fresh_csrf:
                csrf = fresh_csrf
                logger.debug(f"[REQ] CSRF fresh dari {csrf_source_page}: {csrf[:20]}...")
            else:
                # Fallback ke session csrf
                if "/portal/sms/received" in url:
                    csrf = session.get("recv_csrf") or session["csrf"]
                else:
                    csrf = session["csrf"]
                logger.warning(f"[REQ] CSRF scrape gagal, pakai session csrf: {csrf[:20]}...")
        elif method.upper() != "GET":
            # Tidak ada source page di map, pakai session csrf
            if "/portal/sms/received" in url:
                csrf = session.get("recv_csrf") or session["csrf"]
            else:
                csrf = session["csrf"]
        else:
            csrf = session["csrf"]  # GET tidak pakai tapi tetap set untuk konsistensi
...
def numbers_export():
    ...
    csrf    = _scrape_csrf(scraper, f"{BASE_URL}/portal/numbers/test") if scraper else None
    if not csrf:
...
def numbers_download():
    ...
    csrf = _scrape_csrf(scraper, f"{BASE_URL}/portal/numbers/test")
    if not csrf:
...
def debug_add_number():
    ...
    exp_csrf = None
    if exp_scraper:
        exp_csrf = _scrape_csrf(exp_scraper, f"{BASE_URL}/portal/numbers/test")
        if exp_csrf:
...
def debug_export():
    ...
    fresh_csrf = _scrape_csrf(scraper, f"{BASE_URL}/portal/numbers/test")
    if fresh_csrf:
